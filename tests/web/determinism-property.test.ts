...
describe('Property-based determinism (QUAL-01)', () => {
  it(
    'identical inputs produce identical hashes after 500+ ticks (QUAL-01)',
    () => {
      fc.assert(
        fc.property(
          ...
          (ticksBefore, buildPlacements, ticksAfter) => {
            ...
            // 6. Compare hashes
            const serverHash =
              room.createDeterminismCheckpoint().hashHex;
            const clientHash =
              sim.createLocalCheckpoint()!.hashHex;
  ...
  it(
    'determinism holds with interleaved builds from both teams',
    () => {
      fc.assert(
        fc.property(
          ...
          (ticksBefore, buildPlacements, ticksAfter) => {
            const { room, templates, team1, team2 } =
              setupServerAndClient(52, 52);
  ...
  it(
    'determinism holds with destroy inputs',
    () => {
      fc.assert(
        fc.property(
          fc.integer({ min: 500, max: 550 }),
