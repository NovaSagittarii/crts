...
describe('reconnect input-log replay (RECON-01)', () => {
  ...
  test('reconnect with queued builds includes input log entries from snapshot tick forward (RECON-01)', async ({
    connectedRoom,
    startLockstepMatch,
    connectClient,
  }) => {
  ...
  test('reconnect without pending events produces empty inputLog (RECON-01)', async ({
    connectedRoom,
    startLockstepMatch,
    connectClient,
  }) => {
  ...
  test('no full state broadcast emitted to room after reconnect in input-only mode (RECON-01)', async ({
    connectedRoom,
    startLockstepMatch,
    connectClient,
  }) => {
    ...
    const stateObserver = observeEvents<RoomStatePayload>(match.host, 'state');
