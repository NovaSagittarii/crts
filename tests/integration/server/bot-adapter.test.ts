...
describe('bot:add event', () => {
  test(
    'host can add bot to empty slot',
    async ({ connectClient }) => {
...
describe('bot client lifecycle', () => {
  test(
    'bot client connects and joins lobby like a human',
    async ({ integration }) => {
  ...
  test(
    'bot completes full match lifecycle',
    async ({ integration }) => {
      const { port, connectClient } = integration;
  ...
  test(
    'bot membership shows isBot flag and displayName',
    async ({ integration }) => {
      const { port, connectClient } = integration;
