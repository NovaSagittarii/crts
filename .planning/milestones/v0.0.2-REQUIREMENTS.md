# Requirements Archive: v0.0.2 Gameplay Expansion
...
- ✓ `STRUCT-01`: Template-wide integrity checks consume HP and restore deterministically.
- ✓ `STRUCT-02`: Players can queue owned-structure destroy actions with authoritative outcomes.
- ✓ `BASE-01`: Canonical 5x5 base footprint (16 occupied cells) is used consistently for breach gameplay.
- ✓ `BUILD-01`: Placement legality requires full-footprint coverage inside union build zones.
- ✓ `BUILD-02`: Build-zone radius is fixed at 15 for the milestone.
- ✓ `XFORM-01`: Rotate control is supported before preview/queue placement.
- ✓ `XFORM-02`: Mirror control is supported before preview/queue placement.
- ✓ `QUAL-03`: Preview, queue, and simulation placement outcomes are consistent for transformed and non-transformed templates.
- ✓ `UI-01`: Lobby and in-game screens are separated by authoritative match-state transitions.
- ✓ `UI-02`: In-match pan/zoom keeps placement and interaction targeting accurate.
- ✓ `UI-03`: Structure hover and pin interactions expose details and actions.
- ✓ `UI-04`: Economy/build/team overlays are available adjacent to gameplay.
- ✓ `UI-05`: Local-team union build-zone outline is visible during placement.
- ✓ `QUAL-04`: Two-client and reconnect scenarios preserve deterministic structure/build/destroy outcomes.
...
## v1 Requirements (Archived Snapshot)
...
_Archived: 2026-03-03 after v0.0.2 milestone completion_
