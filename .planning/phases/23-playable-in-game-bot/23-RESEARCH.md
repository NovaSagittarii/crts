...
const planesTensor = tf.tensor3d(planes, [C, H, W]).transpose([1, 2, 0]).expandDims(0);
...
// Dispose tensors
planesTensor.dispose(); scalarsTensor.dispose();
logits.dispose(); value.dispose();
```
...
const authPayload = socket.handshake.auth && typeof socket.handshake.auth === 'object'
