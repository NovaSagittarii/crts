...
function computeGAE(
  ...
  for (let t = T - 1; t >= 0; t--) {
    ...
    const nextNonTerminal =
      t === T - 1 ? (dones[T - 1] ? 0 : 1) : dones[t] ? 0 : 1;
...
function sampleMaskedAction(
  ...
  return tf.tidy(() => {
    ...
    const maskedLogits = tf.add(logits, tf.mul(tf.sub(1, maskTensor), -1e9));

    // Sample action from masked distribution
    const action = tf.multinomial(maskedLogits.expandDims(0), 1).dataSync()[0];
...
async function loadCheckpoint(checkpointDir: string): Promise<tf.LayersModel> {
