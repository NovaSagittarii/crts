...
Phase 21 already has `mineSequencePatterns()` using PrefixSpan for sequential patterns. For frequent _sets_ (order-independent), a simpler approach is sufficient:
...
function extractTemplateEncounters(
  ...
  const filteredTicks = tickRange
    ? match.ticks.filter(
        (t) => t.tick >= tickRange.start && t.tick < tickRange.end,
      )
    : match.ticks;
  ...
  for (const tick of filteredTicks) {
    for (const action of tick.actions) {
      if (
        ...
        if (!teamBuilds.has(action.teamId))
          teamBuilds.set(action.teamId, new Map());
