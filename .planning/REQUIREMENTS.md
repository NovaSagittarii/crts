...
- [x] **REF-04**: Build preview, queue validation, and build apply flows use the same `GridView`-backed geometry pipeline.
...
- [x] **REF-06**: Duplicate template/offset-template logic in `packages/rts-engine/rts.ts` is removed without changing authoritative rejection reasons, resource costs, or applied outcomes.
